...
def Get_IQ_Data_Ascii(MLEN=5000):
    CSVd = []
...
def printData(iqData):
    f = open('output.csv', 'w')
    f.write('I-Data,Q-Data\n')
    numIQ = int(len(iqData) / 2) - 1
    for i in range(numIQ):
        outStr = f'{iqData[2 * i]},{iqData[2 * i + 1]}\n'
        f.write(outStr)
    f.close()
...
FSW.write(':SENS:SWE:TIME 0.000100')        # Capture time
